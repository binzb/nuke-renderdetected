...
def get_plugin():
    ...
    filename = 'plugin.txt' # txt文件和当前脚本在同一目录下，所以不用写具体路径
    pos = []
...
def DeadlineErrorDetection(nodes):
    ...
    SOURCE2 = ''
    SOURCE3_Plugin = get_plugin()#['OFXuk.co.thefoundry.tinderbox','WorldPos_Lambert_Shader_ik','AFXGlow.gizmo']    #Plugin not in server
    SOURCE4 = ['.####.']
    ...
    for s2 in nodeList:
        if s2.Class() != 'Read':
            for i in SOURCE3_Plugin:
                if i == s2.Class() and s2.name() not in ErrorPlugin:
